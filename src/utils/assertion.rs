use solana_program::{
    account_info::AccountInfo, entrypoint::ProgramResult, msg, program_error::ProgramError, pubkey::Pubkey, sysvar
};

use crate::error::TokenWrapperError;

use super::{get_reserve_authority, get_token_freeze_authority, get_token_mint_authority, get_vanilla_token_mint};
...
pub fn assert_vanilla_token_mint(token_2022_mint: Pubkey, program_id: Pubkey, actual_vanilla_token_mint: Pubkey) -> ProgramResult {
    let (expected_vanilla_token_mint, _, _) =
        get_vanilla_token_mint(token_2022_mint, program_id);
...
pub fn assert_reserve_authority(token_2022_mint: Pubkey, program_id: Pubkey, actual_reserve_authority: Pubkey) -> ProgramResult {
    let (expected_reserve_authority, _, _) =
...
pub fn assert_mint_authority(token_mint: Pubkey, program_id: Pubkey, actual_mint_authority: Pubkey) -> ProgramResult {
    let (expected_mint_authority, _, _) =
        get_token_mint_authority(token_mint, program_id);
...
pub fn assert_freeze_authority(token_mint: Pubkey, program_id: Pubkey, actual_freeze_authority: Pubkey) -> ProgramResult {
    let (expected_freeze_authority, _, _) =
        get_token_freeze_authority(token_mint, program_id);
...
pub fn assert_is_account_uninitialized(account: &AccountInfo) -> ProgramResult {
    ...
    assert_with_msg(account.data_is_empty() && **account_lamports == 0u64, TokenWrapperError::UnexpectedInitializedAccount, "The account is already initialized, expected to be uninitialized")
}

pub fn assert_is_account_initialized(account: &AccountInfo) -> ProgramResult {
    let account_lamports = account.try_borrow_lamports()?;

    assert_with_msg(**account_lamports > 0u64, TokenWrapperError::ExpectedInitializedAccount, "The account is not initialized, expected to be initialized")
}
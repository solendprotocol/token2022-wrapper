use solana_program::{
    account_info::{next_account_info, AccountInfo}, entrypoint::ProgramResult, msg, program::{invoke, invoke_signed}, program_pack::Pack, pubkey::Pubkey, rent::Rent, system_program, sysvar::Sysvar
};

use crate::{error::TokenWrapperError, instruction::TokenWrapperInstruction, utils::{assert_rent, assert_system_program, assert_token_program, assert_with_msg, get_token_freeze_authority, get_token_mint_authority, get_vanilla_token_mint}};
...
pub fn process_initialize_token (
    ...
    let (expected_vanilla_token_mint, _, _) = get_vanilla_token_mint(*token_2022_mint.key, *program_id);
    ...
    let token_2022_mint_data = spl_token_2022::state::Mint::unpack(&unwrapped_token_2022_mint_data)?;

    assert_with_msg(expected_vanilla_token_mint == *vanilla_token_mint.key, TokenWrapperError::UnexpectedVanillaToken, "Invalid vanilla token mint passed")?;

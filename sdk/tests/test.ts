...
const main = async() => {
    ...
    let initializeIx = token2022WrapperSdk.createInitializeWrapperTokenInstruction(
        userB.publicKey,
        token2022Mint
    );
    ...
    let initializeSig = await web3.sendAndConfirmTransaction(connection, initializeTx, [userB]);
    console.log(`Wrapper tokens initialized: https://solscan.io/tx/${initializeSig}?cluster=devnet`);
    ...
    // Should be X
    let userT22BalanceBeforeDeposit = (await connection.getTokenAccountBalance(userBToken2022Account)).value.uiAmount;
    ...
    let depositIx = await token2022WrapperSdk.createDepositAndMintWrapperTokensInstruction(
        userB.publicKey,
    ...
    let depositSig = await web3.sendAndConfirmTransaction(connection, depositTx, [userB]);
    console.log(`Wrapper tokens minted: https://solscan.io/tx/${depositSig}?cluster=devnet`);
    ...
    // Should be X - 100
    let userT22BalanceAfterDeposit = (await connection.getTokenAccountBalance(userBToken2022Account)).value.uiAmount;
    // Should be 99
    let userWrapperBalanceAfterDeposit = (await connection.getTokenAccountBalance(userBWrapperTokenAccount)).value.uiAmount;

    console.log("User T22 balance after deposit: ", userT22BalanceAfterDeposit);
    console.log("User wrapper balance after deposit: ", userWrapperBalanceAfterDeposit);

    let withdrawAmount = new BN(99).mul(new BN(10 ** token2022Decimals))

    let withdrawIx = await token2022WrapperSdk.createWithdrawAndBurnWrapperTokensInstruction(
        userB.publicKey,
        userBToken2022Account,
        token2022Mint,
        withdrawAmount
    ...
    let withdrawSig = await web3.sendAndConfirmTransaction(connection, withdrawTx, [userB]);
    console.log(`Wrapper tokens burned: https://solscan.io/tx/${withdrawSig}?cluster=devnet`);

    // Should be X - 1.99
    let userT22BalanceAfterWithdraw = (await connection.getTokenAccountBalance(userBToken2022Account)).value.uiAmount;
    // Should be 0
    let userWrapperBalanceAfterWithdraw = (await connection.getTokenAccountBalance(userBWrapperTokenAccount)).value.uiAmount;

    console.log("User T22 balance after withdraw: ", userT22BalanceAfterWithdraw);
    console.log("User wrapper balance after withdraw: ", userWrapperBalanceAfterWithdraw);

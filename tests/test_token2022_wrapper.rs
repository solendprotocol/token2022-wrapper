...
pub async fn create_and_mint_tokens(
    ...
    let token_mint = create_mint(client, &payer_keypair.pubkey(), None, decimals, None).await.unwrap();

    let associated_token_account = create_associated_token_account(client, recipient, &token_mint, &spl_token::id()).await.unwrap();

    let _ = mint_tokens(client, &payer_keypair, &token_mint, &associated_token_account, amount, None).await;
...
pub async fn create_and_mint_tokens_token_2022(
    ...
    let token_mint = create_token_2022_mint(client, &payer_keypair.pubkey(), None, decimals, None).await.unwrap();

    let token_account = create_token_account_token_2022(client, &recipient, &token_mint).await.unwrap();

    let _ = mint_token_2022_tokens(client, &payer_keypair, &token_mint, &token_account, amount, None).await;
...
async fn test_1() {
    ...
    let (token_2022_mint, user_token_2022_account) = create_and_mint_tokens_token_2022(&mut test_client, &user.pubkey(), amount_2022, decimal_2022).await;

    let user_token_2022_balance = get_token_balance(&mut test_client, &user_token_2022_account).await;

    println!("amount_2022: {}", amount_2022);
    println!("user_token_2022_balance: {}", user_token_2022_balance);

    assert_with_msg(amount_2022 == user_token_2022_balance, "Invalid user token_2022 balance");
    
    let token_2022_data = get_token_mint(&mut test_client, &token_2022_mint).await.unwrap();

    assert_with_msg(token_2022_data.decimals == decimal_2022, "Invalid token_2022 decimals");

    let initialize_ix = create_initialize_token_instruction(&payer_keypair.pubkey(), &token_2022_mint);
